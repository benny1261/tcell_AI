'''souce:https://github.com/gxwangupc/HybridSN'''

import keras
from keras.layers import Conv2D, Conv3D, Flatten, Dense, Reshape, BatchNormalization
from keras.layers import Dropout, Input
from keras.models import Model # Buggy
...
class Config():
    ...
    def init(self):
        ...
        self.parser.add_argument('-DATASET', default='Indian', choices=['Indian','T'],
        ...
        self.parser.add_argument('-N_CLS', type=int, default=4, choices=[16, 16, 9],
                                 help='how many class in the training data')
        self.parser.add_argument('-EPOCH', type=int, default=10, help='how many epochs to train for')
        self.parser.add_argument('-BATCH_SIZE', type=int, default=64, help='training batch size')  #init 256
        ...
        self.parser.add_argument('-use_PCA', type=bool, default=True,
                                 help='use PCA or not')
        self.parser.add_argument('-TEST_RATIO', type=float, default=0.1, help='ratio of testing data in the full set')

        self.parser.add_argument('-DATA_DIR', default='data/', help='directory to load data')
...
def load(dataset):
    if dataset == 'Indian':
        ...
        data = loadmat(os.path.join(opt.DATA_DIR, 'data_x.mat'))['data_x']
        label = loadmat(os.path.join(opt.DATA_DIR, 'data_y.mat'))['data_y']
    else:
...
def split(data, label):
    data_tr, data_te, label_tr, label_te = train_test_split(data, label, test_size=opt.TEST_RATIO, random_state=345, stratify=label)
...
# reshape = Reshape((conv3d_3._keras_shape[1], conv3d_3._keras_shape[2], conv3d_3._keras_shape[3]*conv3d_3._keras_shape[4]))(conv3d_3) # Buggy
reshape = Reshape((conv3d_3.shape[1], conv3d_3.shape[2], conv3d_3.shape[3]*conv3d_3.shape[4]))(conv3d_3)
